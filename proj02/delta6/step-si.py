...
for conc in OPTIONS:
    ...
    BCT_OPTIONS = np.linspace(0.98, 1.02, 5)
    HCP_OPTIONS = np.linspace(0.96, 1.00, 5)
    ...
    for opt in BCT_OPTIONS:
        ...
        alloys.append(Alloy(1, 'Si', conc, 1.0))

        alloys.append(Alloy(2, 'Fe', fe, -1.0))
        alloys.append(Alloy(2, 'Mn', mn, -1.0))
        alloys.append(Alloy(2, 'Si', conc, -1.0))

        calc = EMTO()
        calc.set(dir='work-{1}/conc-{2}/opt-{0:0.4f}/bct'.format(opt,id,conc),
                 lat = 6, # fcc
                 ncpa=20,
                 amix=0.05,
    ...
    for opt in HCP_OPTIONS:
     ...
     #   c0 = np.sqrt(8 / 3.0) * a0
        ...
        alloys.append(Alloy(1, 'Mn', mn / 2, -1.0))
        alloys.append(Alloy(1, 'Si', conc / 2, 1.0))
        alloys.append(Alloy(1, 'Si', conc / 2, -1.0))
        alloys.append(Alloy(1, 'Fe', fe / 2, 1.0))
        alloys.append(Alloy(1, 'Fe', fe / 2, -1.0))

        calc = EMTO()
        calc.set(dir='work-{1}/conc-{2}/opt-{0:0.4f}/hcp'.format(opt, id,conc),
                 lat=4,  # fcc
                 ncpa=20,
                 amix=0.05,
                 afm='F',  # ferromagnetic calculation
    ...
    save(result_all, '{0} | {1} | {2} {3} {4} {5} {6} {7} {8}'.format(bct_energies, hcp_energies, bct_v0, bct_e0, bct_B, hcp_v0, hcp_e0, hcp_B, hcp_e0-bct_e0))
