...
for row in buffer:
    ...
    if id%size==rank:
        ...
        save(result, 'hcp calculate {0}'.format(id))
        ...
        for opt in OPTIONS:
            ...
            if ni > 1e-7:
                alloys.append(Alloy(1, 'Ni', ni , .0))
            if cr > 1e-7:
                alloys.append(Alloy(1, 'Cr', cr , .0))
            if al > 1e-7:
                alloys.append(Alloy(1, 'Al', al , .0))
            if si > 1e-7:
                alloys.append(Alloy(1, 'Si', si , .0))
            if mo > 1e-7:
                alloys.append(Alloy(1, 'Mo', mo , .0))
            if co > 1e-7:
                alloys.append(Alloy(1, 'Co', co , .0))
            if cu > 1e-7:
                alloys.append(Alloy(1, 'Cu', cu , .0))
            if nb > 1e-7:
                alloys.append(Alloy(1, 'Nb', nb , .0))
            if ti > 1e-7:
                alloys.append(Alloy(1, 'Ti', ti , .0))
            if v > 1e-7:
                alloys.append(Alloy(1, 'V', v , .0))
            if w > 1e-7:
                alloys.append(Alloy(1, 'W', w , .0))
